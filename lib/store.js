...
function commitUpload({ rows, mapping, filename, defaultPeriod }) {
  ...
  rows.forEach((row, rowIdx) => {
    ...
    if (!creativeId && dims.name) {
      // A bare name isn't always a unique identity — some exports (media-mix
      // reports especially) repeat the same creative name across many
      // breakdown rows (channel, audience, tactic, ad group, ...). Fold in
      // whatever distinguishing context the row actually has so those don't
      // collide and silently overwrite each other; this is still stable
      // across re-uploads of the same file, so corrections still update
      // rather than duplicate.
      const context = [dims.name, dims.campaign_name, dims.adset_name, dims.audience, dims.channel, dims.platform, dims.channel_type, dims.phase, dims.creative_size]
        .filter(Boolean).join("|");
      creativeId = `custom-${slugify(context)}`;
    }
    if (!creativeId) {
...
/** Remove every fact row for a given creative_id (e.g. cleaning up a test/
 * mistaken upload) and drop the creative dimension entry if nothing
 * references it anymore. */
function deleteFactsForCreative(creativeId) {
  ensureSeeded();
  const facts = readJson(filePath("facts"), []);
  const remaining = facts.filter((f) => f.creative_id !== creativeId);
  const removed = facts.length - remaining.length;
  writeJson(filePath("facts"), remaining);
  const stillReferenced = remaining.some((f) => f.creative_id === creativeId);
  if (!stillReferenced) {
    const creatives = readJson(filePath("creatives"), {});
    if (creatives[creativeId]) { delete creatives[creativeId]; writeJson(filePath("creatives"), creatives); }
  }
  return { creativeId, removed, totalFacts: remaining.length };
}
